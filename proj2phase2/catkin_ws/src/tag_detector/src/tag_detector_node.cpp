...
cv::Mat K, D;  // intrinsic parameters, K for camera matrix, D for distortion coefficients
...
void process(const vector<int> &pts_id, const vector<cv::Point3f> &pts_3, const vector<cv::Point2f> &pts_2, const ros::Time& frame_time){
    ...
    for(unsigned int i=0;i<pts_id.size();i++){
        int row1=i*2, row2 = row1+1;
        A(row1,0) = pts_3[i].x;
        A(row1,1) = pts_3[i].y;
        A(row1,2) = 1;
        A(row1,3) = 0;
    ...
    Matrix3d HH;
    HH<<one,two,three;
    JacobiSVD<MatrixXd> svd2(HH, ComputeThinU | ComputeThinV);
    ...
    // K^(-1)*H ~= (R,t), get an estimated R
    //cv::Mat KH = K.inv() * H;
    // get the real R by SVD
    //JacobiSVD<MatrixXd> svd(A, ComputeThinU | ComputeThinV);
